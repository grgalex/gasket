...
def gen_histogram(data1, data2, histogram_filename):
    ...
    _, edges, _ = ax.hist(data1, bins=nu_bins, color='blue', alpha=0.5, label='GASKET')

    # Plot second histogram (blue)
    # ax.hist(charon_zeros, bins=[-0.5, 0.5], color='blue', alpha=0.5, label='CHARON')
    zero_bins = [-0.5, 0.5]
    ax.hist(data2, bins=edges, color='red', alpha=0.5, label='CHARON')
    ax.hist(charon_zeros, bins=zero_bins, color='red', alpha=0.5, label='CHARON')

    # Grid and legend
    # ax.grid(True, color='gray', linestyle='--', linewidth=0.5)
    ax.legend()
    
    # x_ticks = [0] + [int(t) for t in edges[2:]]
    x_ticks = [0] + edges 
    plt.xticks(x_ticks)
    ...
    plt.ylim(0, np.max(np.histogram(data1, bins=nu_bins)[0]))  # Limit y-axis to the maximum frequency
